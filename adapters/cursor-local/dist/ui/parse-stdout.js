 ...
 */
function formatShellToolResultForLog(result) {
...
function compactShellToolInput(rawInput, payload) {
    const cmd = asString(payload?.command ?? asRecord(rawInput)?.command);
    if (cmd)
        return { command: cmd };
    return rawInput;
}
function parseUserMessage(messageRaw, ts) {
    ...
    if (directText)
        entries.push({ kind: "user", ts, text: directText });
...
function parseAssistantMessage(messageRaw, ts) {
    if (typeof messageRaw === "string") {
        const text = messageRaw.trim();
    ...
    for (const partRaw of content) {
        ...
        if (type === "tool_call") {
            ...
            const input = name === "shellToolCall" || name === "shell"
        ...
        if (type === "tool_result") {
            ...
            const contentText = typeof rawOutput === "object" && rawOutput !== null
...
function parseCursorToolCallEvent(event, ts) {
...
export function parseCursorStdoutLine(line, ts) {
    ...
    if (type === "result") {
        ...
        const cachedTokens = asNumber(usage?.cached_input_tokens, asNumber(usage?.cachedInputTokens, asNumber(usage?.cache_read_input_tokens)));
