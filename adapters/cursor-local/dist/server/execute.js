...
function resolveCursorBiller(env, billingType, provider) {
...
export async function ensureCursorSkillsInjected(onLog, options = {}) {
    ...
    const removedSkills = await removeMaintainerOnlySkillSymlinks(skillsHome, skillsEntries.map((entry) => entry.runtimeName));
    for (const skillName of removedSkills) {
        await onLog("stderr", `[paperclip] Removed maintainer-only Cursor skill "${skillName}" from ${skillsHome}\n`);
    }
    ...
    for (const entry of skillsEntries) {
        ...
        try {
            ...
            if (result === "skipped")
                continue;
            await onLog("stderr", `[paperclip] ${result === "repaired" ? "Repaired" : "Injected"} Cursor skill "${entry.key}" into ${skillsHome}\n`);
        }
        catch (err) {
            await onLog("stderr", `[paperclip] Failed to inject Cursor skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`);
        }
    }
}
export async function execute(ctx) {
    ...
    if (runtimeSessionId && !canResumeSession) {
        await onLog("stdout", `[paperclip] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`);
    }
    ...
    if (sessionId &&
        ...
        await onLog("stdout", `[paperclip] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`);
