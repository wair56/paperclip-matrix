...
function printUserMessage(messageRaw) {
    ...
    if (directText)
        console.log(pc.gray(`user: ${directText}`));
...
function printAssistantMessage(messageRaw) {
    ...
    const directText = asString(message.text).trim();
    if (directText)
        console.log(pc.green(`assistant: ${directText}`));
    const content = Array.isArray(message.content) ? message.content : [];
    for (const partRaw of content) {
        const part = asRecord(partRaw);
        if (!part)
        ...
        if (type === "tool_result") {
            ...
            if (contentText)
                console.log((isError ? pc.red : pc.gray)(contentText));
        }
    }
}
...
export function printCursorStreamEvent(raw, _debug) {
    ...
    if (type === "result") {
        ...
        const cached = asNumber(usage?.cached_input_tokens, asNumber(usage?.cachedInputTokens, asNumber(usage?.cache_read_input_tokens)));
        ...
        if (resultText)
            console.log((isError ? pc.red : pc.green)(`assistant: ${resultText}`));
