...
function collectTextEntries(messageRaw, ts, kind) {
    ...
    if (directText)
        entries.push({ kind, ts, text: directText });
    ...
    for (const partRaw of content) {
        ...
        if (type === "tool_result" || type === "tool_response") {
            const toolUseId = asString(part.tool_use_id) ||
                ...
                asString(part.id) ||
                "tool_result";
            const contentText = asString(part.output) ||
                asString(part.text) ||
...
function parseTopLevelToolEvent(parsed, ts) {
...
function readSessionId(parsed) {
    return (asString(parsed.session_id) ||
...
function readUsage(parsed) {
    ...
    return {
        ...
        cachedTokens: asNumber(source.cached_input_tokens, asNumber(source.cachedInputTokens, asNumber(source.cachedContentTokenCount))),
    };
}
export function parseGeminiStdoutLine(line, ts) {
