...
function printTextMessage(prefix, colorize, messageRaw) {
    if (typeof messageRaw === "string") {
        ...
        if (text)
            console.log(colorize(`${prefix}: ${text}`));
        return;
    }
    ...
    for (const partRaw of content) {
        ...
        if (type === "tool_call") {
            ...
            if (input !== undefined)
                console.log(pc.gray(stringifyUnknown(input)));
            continue;
        }
...
function printUsage(parsed) {
    ...
    const cached = asNumber(source.cached_input_tokens, asNumber(source.cachedInputTokens, asNumber(source.cachedContentTokenCount)));
...
export function printGeminiStreamEvent(raw, _debug) {
    const line = raw.trim();
    if (!line)
        return;
    let parsed = null;
