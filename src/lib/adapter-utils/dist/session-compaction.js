...
export const ADAPTER_SESSION_MANAGEMENT = {
    claude_local: {
...
export function getAdapterSessionManagement(adapterType) {
    if (!adapterType)
        return null;
    return ADAPTER_SESSION_MANAGEMENT[adapterType] ?? null;
}
export function readSessionCompactionOverride(runtimeConfig) {
    ...
    if (maxSessionRuns !== undefined)
        explicit.maxSessionRuns = maxSessionRuns;
...
export function resolveSessionCompactionPolicy(adapterType, runtimeConfig) {
